...
class ChainWeb3 {
  ...
  chainConnected(type, address) {
    ...
    setTimeout(() => {
      ...
      if (this.ethereum.chainId) {
        ...
        if (type === 'u')
          this.handleNewAccounts([this.ethereum.selectedAddress]);
      } else {
        this.web3.eth
          .getChainId()
          .then((chainId) => {
            type === 'particle' && this.handleNewAccounts([address]);
  ...
  handleNewAccounts(newAccounts) {
    ...
    if (newAccounts && newAccounts.length > 0) {
      ...
      chainWeb3.myWallets[chainWeb3.account.toLowerCase()] =
        new Date().getTime();
  ...
  handleEventLog(abi, receipt, contractName, eventName) {
    ...
    let result = this.web3Util.parseEventLog(
      this.web3,
      abi,
      receipt,
      eventName
    ...
    let type =
      contractName.toLocaleUpperCase() + '_' + eventName.toLocaleUpperCase();
  ...
  getSelectedAddress() {
    if (this.accounts.length > 0) {
      ...
      this.connector.accounts.length > 0
    ) {
  ...
  isWETH(addr) {
    return (
      addr ==
      ContractsAddr[this.getNetworkVersion()]['WETH'].toLocaleLowerCase()
    );
  }
  ...
  async setupNetwork(chainId) {
    ...
    if (provider) {
      ...
      try {
        ...
        if (switchError.code === 4902) {
          try {
            ...
            console.error(
              'setupNetwork Failed to setup the network--->',
              chainId,
              error
            );
            return {
      ...
      console.error(
        "Can't setup the Block network on metamask because window.ethereum is undefined"
      );
      return {
        code: 401,
        flag: false,
